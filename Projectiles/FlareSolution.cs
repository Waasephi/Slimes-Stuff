...
{
	public class FlareSolution : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Flare Spray");
		}
		...
		public override void AI()
		{
			...
			int dustType = DustType<Dusts.FlareSolution>();
			...
			for (int k = i - size; k <= i + size; k++)
			{
				for (int l = j - size; l <= j + size; l++)
				{
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size))
					{
						...
						if (wall != 0)
						{
							Main.tile[k, l].wall = (ushort)WallType<PhoenixStoneWall>();
						...
						{
							Main.tile[k, l].type = (ushort)TileType<PhoenixStone>();
